...
def main():
    ...
    if target.exists() and json.loads(target.read_text())["fonte"]["tipo"] == "relatorio":
        print("Íntegra já integrada; fonte parcial não sobrescreve o relatório.")
        return
    registry = (FOLDER / "registro-tse.txt").read_text()
    for text in ["BR-06004/2026", "17/09/2026", "20/09/2026", "(31%)", "(42%)", "(27%)"]:
        assert text in registry, text
    old = json.loads((ROOT / "analysis/reponderacao/pesquisas/quaest_2026-09-13.json").read_text())
    ...
    poll.update(
        id="quaest_2026-09-20", registro_tse="BR-06004/2026",
        campo={"inicio": "2026-09-17", "fim": "2026-09-20"},
        divulgacao="2026-09-21", n=2004,
        fonte={
            "tipo": "capturas_fornecidas_usuario", "rotulo": "Quaest · G1 (prints, 1º turno)",
            "url": URL, "pdf": None, "paginas": {},
            "arquivo": "docs/assets/quaest_210926_fontes.json",
            "localizador": "Coluna 21/09 dos quatro prints: geral e três faixas de renda",
            "status": "Fonte parcial: G1 + cotas registradas no TSE.", "nota": note,
            "complementos": [
                {"url": f"fontes/quaest_21092026/{p.name}", "rotulo": p.stem.replace("g1-", "Print ")}
                for p in sorted(FOLDER.glob("*.png"))
            ] + [{"url": "fontes/quaest_21092026/registro-tse.txt", "rotulo": "Registro BR-06004/2026"}],
        },
        renda={
            "unidade": "salarios_minimos", "ano_referencia": 2026,
            "faixas": old["renda"]["faixas"], "amostra_pct": PROFILE,
        ...
        cruzamentos={"1t": {"opcoes": OPTIONS, "linhas": ROWS,
        ...
        sem_cruzamento={"2t": "21/09: ainda sem cruzamento de renda do 2º turno no acervo; "
    ...
    manifest = {
        "registro_tse": poll["registro_tse"], "origem": "Quatro imagens fornecidas pelo usuário",
        "url_indicada": URL, "recebido_em": "2026-09-21", "coluna": "21/09",
        "opcoes": OPTIONS, "publicado": PUBLISHED, "renda": ROWS,
        "cotas_registradas_pct": PROFILE,
        "recomposto": {k: round(sum(w * r[j] for w, r in zip(PROFILE, ROWS)) / 100, 2)
                       for j, k in enumerate(OPTIONS)},
        "nota": note,
        "arquivos": [{"arquivo": p.name, "sha256": hashlib.sha256(p.read_bytes()).hexdigest()}
                     for p in sorted(FOLDER.iterdir()) if p.suffix in {".png", ".txt"}],
    }
    (ROOT / "docs/assets/quaest_210926_fontes.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
