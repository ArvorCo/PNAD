...
def test_all_versions_visible_but_only_unique_eligible_waves_enter_means():
    ...
    assert sum("1t" in p["turnos"] for p in active.values()) == 2
    ...
    for p in active.values():
        assert all(t["residuo_max"] == 0 for t in p["turnos"].values())
...
def test_institute_panels_show_every_version_in_both_turns():
    ...
    assert len(panels) == 2
    for panel in panels:
        assert "3 ondas · 4 versões" in panel.get_text()
        svg_text = panel.find("svg").get_text()
        for label in ["09/08", "07/09 v1", "07/09 v2", "18/09"]:
