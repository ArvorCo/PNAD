#!/usr/bin/env python3
"""Generate docs/pnad.html from PNADC data.
...
The resulting docs/pnad.html is a self-contained static page suitable for
GitHub Pages (main:/docs). Note: docs/index.html is the hand-written library
hub listing every report and is NOT generated by this script.
"""
...
def _strip_local_paths(value: Any) -> Any:
    """Rewrite absolute local paths as repo-relative ones.

    The dashboard payload records the CSV it was built from. No chart reads that
    field, but embedding it would publish the maintainer's home directory layout
    on a public page, so it is normalised before going into the template.
    """
    root = f"{ROOT}/"
    if isinstance(value, dict):
        return {key: _strip_local_paths(item) for key, item in value.items()}
...
def main() -> int:
    ...
    p.add_argument(
        "--output",
        default=str(OUTPUT),
        help="Output HTML path (default: docs/pnad.html)",
    )
    args = p.parse_args()
